...
if (isset($_POST["usr_delete_id"]) && is_login()) {
...
if (isset($_POST["check_id"]) && is_login()) {
...
if (isset($_POST["admin_delete_id"]) && is_login()) {
...
if (isset($_POST["due_dlt_id"]) && is_login()) {
...
if (isset($_POST["dtl_all_noti"]) && is_login()) {
  sleep(1);
...
if (isset($_POST["rem_si_noti"])  && is_login()) {
...
if (isset($_POST["sgst_dlt_id"]) && is_login()) {
  sleep(1);
...
function is_login()
{
  ...
  $token = $_COOKIE["toawasnsdeaden"];
  $connect = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
  if (isset($id) && isset($pass) && isset($token)) {
    $get = "SELECT * FROM `owner` WHERE id = '$id' && owner_password = '$pass' && owner_token = '$token'";
    if (mysqli_query($connect, $get)) {
      return true;
    } else {
      return false;
    }
