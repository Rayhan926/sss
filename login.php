...
if (
  isset($_COOKIE["toawasnsdeaden"]) || 
  isset($_COOKIE["psasgsfsafsgdsad"]) || 
  isset($_COOKIE["iasasaddffsfd"]) ||
  isset($_COOKIE["t35e6and45easn"]) || 
  isset($_COOKIE["p35edasdd785easd"]) || 
  isset($_COOKIE["i35esda5asd"]) ) {

    $token = $_COOKIE["toawasnsdeaden"];
    $pass = $_COOKIE["psasgsfsafsgdsad"];
    $id = $_COOKIE["iasasaddffsfd"];
    $check_admin = "SELECT * FROM `owner` WHERE owner_password = '$pass' && owner_token = '$token' && owner_action = 'unblock' && id = '$id' ";
...
if (
  ...
  isset($_COOKIE["i35esda5asd"])) {
    ...
    $usr_tokn = $_COOKIE['t35e6and45easn'];
    $usr_pass = $_COOKIE['p35edasdd785easd'];
    $usr_id = $_COOKIE['i35esda5asd'];
    $check_usr = "SELECT * FROM `users` WHERE user_password = '$usr_pass' && user_token = '$usr_tokn' && user_action = 'unblock' && id = '$usr_id' ";
    $run_user_check = mysqli_query($connect, $check_usr);
